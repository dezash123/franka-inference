...
#error Tail-subgroup skip requires an independent, barrier-free microkernel.
#endif
    if (tn+ln*ugemm_gateup_sg_tile_n >= tokens) return;
    ...
    for (int j=0; j<ugemm_gateup_sg_tile_n; j+=16) {
        const int token=tn+ln*ugemm_gateup_sg_tile_n+j+get_sub_group_local_id();
        token_scales[j/16]=token<tokens ? convert_float(sx[token]) : 0.0f;
    }
    if (tn+(ln+1)*ugemm_gateup_sg_tile_n <= tokens) {
    #pragma unroll 32
    for (int j = 0; j < ugemm_gateup_sg_tile_n; ++j) {
        ...
        if (1) {
            ...
            #endif
            float2 g = (convert_float2(gv)*sub_group_broadcast(token_scales[j/16],j%16))*(float2)(gate_scales[0],gate_scales[1]);
            float2 u = (convert_float2(uv)*sub_group_broadcast(token_scales[j/16],j%16))*(float2)(up_scales[0],up_scales[1]);
        ...
        for (int i = 0; i < ugemm_gateup_sg_tile_m/2; i += 16) {
            ...
            if (1) {
                ...
                #endif
                float g = (convert_float(gv)*sub_group_broadcast(token_scales[j/16],j%16))*gate_scales[i/16];
                float u = (convert_float(uv)*sub_group_broadcast(token_scales[j/16],j%16))*up_scales[i/16];
