...
cmd = [
    ...
    "--fuse-qkv", "vision,language,expert", "--batch-vision",
    "--align-vision-mlp", "--fuse-vision-padding",
    "--config", json.dumps(config),
    "--validation-samples", "zero,random,validation-0,validation-1,validation-2,validation-3,validation-4,validation-5",
]
if args.float_gate:
    cmd += ["--require-float-gate", args.float_gate]
elif args.variant in ("reuse", "gemm") and not args.allow_roundoff:
    cmd += ["--require-bitwise", "custom-per-token-fusedpad"]
if args.gateup_folder:
...
if not args.float_gate:
    for sample, checks in result["sanity_checks"].items():
        check = checks["custom-per-token-fusedpad"]
