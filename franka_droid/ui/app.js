...
$('refresh-log').onclick=async()=>{try{$('log').textContent=(await(await fetch('/api/log')).json()).text;}catch(e){error(e.message);}};
...
$('external-label').textContent=r.external_view&&motion?r.external_view.serial+' · left eye':'Random camera · left eye';$('receipt').textContent=r.receipt||'No session started from this panel.';
...
for(const view of ['external','wrist']){$(view).onload=()=>$(view).classList.add('loaded');$(view).onerror=()=>$(view).classList.remove('loaded');}
function frames(){if(connected)for(const view of ['external','wrist'])$(view).src='/api/frame?view='+view+'&t='+Date.now();setTimeout(frames,2200);}
poll();frames();
