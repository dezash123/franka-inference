...
The panel shows External 1, External 2 and Wrist independently, with serial labels, measured capture FPS and stale/disconnected states. Previews refresh once per second; FPS measures accepted camera frames, not browser refreshes or inference. At idle, the panel proxies the existing camera viewer. During warm-up and arm sessions, `camera_preview.py` reuses the inference-owned streams on a housekeeping thread and publishes bounded JPEG/status snapshots. Configured external cameras excluded from inference are opened for preview only by that same owner; their failures do not enter required-camera checks. The idle viewer remains stopped while the inference process owns the cameras. No preview work runs in the 1 kHz control callback.
...
Camera API: `GET /api/cameras` returns stable serial IDs, role, measured FPS, freshness/health, inference inclusion, and frame availability. `GET /api/frame?camera=<serial>` returns that camera's left-eye JPEG. The older `view=external|wrist` route remains available for compatibility. Unknown IDs, unavailable or stale frames return an error rather than an old image labeled live.
