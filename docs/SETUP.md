...
The effective inventories in `requirements/` and `manifests/python-*-effective.json` resolve package precedence; the raw inventories also contain shadowed duplicate versions. `manifests/*-module-paths.json` shows actual module origins. `manifests/python-path-bridges.json` preserves the exact five local `.pth` bridges, including the `site.addsitedir` calls needed to process nested paths. The inventories are a record, **not** a standalone `pip install -r` recipe: local OpenPI/ROS packages, XPU wheels, package indexes and the Transformers replacement files must be supplied separately.
...
`deploy/kernel/config-6.18.53-spring-rt` preserves the installed kernel configuration, including `CONFIG_PREEMPT_RT=y`. The kernel binary, modules, initramfs and bootloader are not in Git. The exact image/header/libc packages, version `6.18.53-1`, remain in `/var/lib/spring-data/kernel-rt-20260923` on Rome; [host-asset recovery](ARTIFACTS.md#kernel-packages-and-openpi-tokenizer) provides their hashes and a download-only helper. Install/provision the kernel, modules, initramfs and boot selection separately during a restore. A `.config` file alone does not install PREEMPT_RT. The local libfranka patch accepts the mainline PREEMPT_RT version signature even when `/sys/kernel/realtime` is absent.
...
`franka_droid/config/setup.json`, `config/droid-fr3-active.json` and calibration files preserve the installed settings. The inference set contains external `39338267` plus wrist ZED Mini `12607895`, both left eye, with no wrist digital zoom. External `37266581` remains excluded from inference, but is captured for the UI preview. Its configuration note describes an earlier shared USB 2 link: after rewiring, the externals were observed on separate USB buses and simultaneous capture of all three was verified at about 15 FPS. The historical note is preserved verbatim in the exported configuration. The selector still supports random selection when multiple external cameras are enabled; this audit did not change the active camera set.
