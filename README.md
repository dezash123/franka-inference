...
All backends return absolute gripper closure (0 open, 1 closed). `runtime_contract.py` prevents mixing action decoding or checkpoints. The newer checkpoint-A adapter is included, with its separate provenance and validation history in [PI05_FINAL_SNAPSHOT.md](docs/PI05_FINAL_SNAPSHOT.md). The older handoff predates that addition. The live application itself remains authoritative for readiness and robot state.
